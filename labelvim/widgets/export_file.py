...
class YOLOConversion:
    ...
    def _convert_to_yolov5_bbox_format(self, bbox, image_width, image_height):
        ...
        x_center, y_center, width, height = xywh2yolo(
    ...
    def _convert_to_yolov5_segmentation_format(
        self, segmentation_points, image_width, image_height
    ):
        """Convert segmentation to YOLOv5 format."""
        yolo_segmentation = cocoseg2yolo(segmentation_points, image_width, image_height)
        return yolo_segmentation

    def _convert_to_yolov8_segmentation_format(
        self, segmentation_points, image_width, image_height
    ):
    ...
    def _convert_to_yolov9_segmentation_format(
        self, segmentation_points, image_width, image_height
    ):
    ...
    def _convert_to_yolov7_segmentation_format(
        self, segmentation_points, image_width, image_height
    ):
        ...
        with open(label_txt_path, "w") as label_txt:
            for annotation in annotations:
                ...
                if self.annotation_type == ANNOTATION_TYPE.BBOX:
                    ...
                    yolov5_bbox = self._convert_to_yolov5_bbox_format(
                        bbox, image_width, image_height
                    )
                    label_txt.write(
                        f"{label} {yolov5_bbox[0]} {yolov5_bbox[1]} {yolov5_bbox[2]} {yolov5_bbox[3]}\n"
                    )
                    if self.include_mask:
                    ...
                    for poly in segmentation:
                        yolov5_segmentation = self._convert_to_yolov5_segmentation_format(
                            poly, image_width, image_height
                        )
    ...
    def _create_zip_file(self):
        ...
        save_path, _ = QFileDialog.getSaveFileName(
            None, "Save Zip File", "yolov5_data.zip", "Zip Files (*.zip)", options=options
        )
        if not save_path:
            return None
        ...
        total_files = (
            len(self.train_file_list) + len(self.valid_file_list) + len(self.test_file_list)
        )
        ...
        with zipfile.ZipFile(save_path, "w") as zipf:
            ...
            for file_name in self.train_file_list:
                ...
                image_path, label_txt_path, mask_path, mask_image_path, mask_instance_path = (
                    self._generate_yolov5_image_labels_and_masks(file_name)
                )
                zipf.write(
                    image_path, os.path.join("train", "images", os.path.basename(image_path))
                )
                zipf.write(
                    label_txt_path,
                    os.path.join("train", "labels", os.path.basename(label_txt_path)),
                )
                if self.include_mask:
                    zipf.write(
                        mask_path, os.path.join("train", "masks", os.path.basename(mask_path))
                    )
                if self.include_img:
                    zipf.write(
                        mask_image_path,
                        os.path.join("train", "masks", os.path.basename(mask_image_path)),
                    )
                if self.include_instance:
                    zipf.write(
                        mask_instance_path,
                        os.path.join("train", "masks", os.path.basename(mask_instance_path)),
                    )
                current_progress += 1
            ...
            for file_name in self.valid_file_list:
                ...
                zipf.write(
                    image_path, os.path.join("valid", "images", os.path.basename(image_path))
                )
                zipf.write(
                    label_txt_path,
                    os.path.join("valid", "labels", os.path.basename(label_txt_path)),
                )
                if self.include_mask:
                    zipf.write(
                        mask_path, os.path.join("valid", "masks", os.path.basename(mask_path))
                    )
                if self.include_img:
                    zipf.write(
                        mask_image_path,
                        os.path.join("valid", "masks", os.path.basename(mask_image_path)),
                    )
                if self.include_instance:
                    zipf.write(
                        mask_instance_path,
                        os.path.join("valid", "masks", os.path.basename(mask_instance_path)),
                    )
                current_progress += 1
                progress_dialog.setValue(current_progress)
                if progress_dialog.wasCanceled():
            ...
            for file_name in self.test_file_list:
                ...
                zipf.write(
                    label_txt_path, os.path.join("test", "labels", os.path.basename(label_txt_path))
                )
                if self.include_mask:
                    zipf.write(
                        mask_path, os.path.join("test", "masks", os.path.basename(mask_path))
                    )
                if self.include_img:
                    zipf.write(
                        mask_image_path,
                        os.path.join("test", "masks", os.path.basename(mask_image_path)),
                    )
                if self.include_instance:
                    zipf.write(
                        mask_instance_path,
                        os.path.join("test", "masks", os.path.basename(mask_instance_path)),
                    )
                current_progress += 1
                progress_dialog.setValue(current_progress)
                if progress_dialog.wasCanceled():
...
class COCOConversion:
    ...
    def _convert_to_coco_format(self, file_list, subset):
        ...
        for file_name in tqdm(file_list, desc=f"Converting {subset}"):
            ...
            if self.include_mask or self.include_img or self.include_instance:
                mask_path, img_path, instance_path = self._generate_masks_and_images(
                    annotation_data, image_info
                )
        ...
        return (
            os.path.join(self.temp_dir, f"{subset}_coco.json"),
            image_path_list,
            mask_list,
            mask_image_list,
            mask_instance_list,
    ...
    def _generate_masks_and_images(self, annotation_data, image_info):
        ...
        if self.include_instance:
            instance_image = np.zeros(
                (image_info["height"], image_info["width"], 3), dtype=np.uint8
            )
        ...
        for annotation in annotation_data["annotations"]:
            ...
            if self.include_mask:
                cv2.rectangle(
                    mask, (bbox[0], bbox[1]), (bbox[0] + bbox[2], bbox[1] + bbox[3]), color, 2
                )
            if self.include_instance:
                # bbox = annotation['bbox']
                cv2.rectangle(
    ...
    def _create_zip_file(self):
        ...
        options = QFileDialog.Options()
        save_path, _ = QFileDialog.getSaveFileName(
            None, "Save Zip File", "coco_data.zip", "Zip Files (*.zip)", options=options
        )
        if not save_path:
            return None
        ...
        with zipfile.ZipFile(save_path, "w") as zipf:
            current_progress = 0
            coco_json_path, image_path_list, mask_list, mask_image_list, mask_instance_list = (
                self._convert_to_coco_format(self.train_file_list, "train")
            )
            ...
            for idx, image_path in enumerate(image_path_list):
                ...
                if self.include_mask:
                    ...
                    zipf.write(
                        mask_list[idx],
                        os.path.join("mask", "train", os.path.basename(mask_list[idx])),
                    )
                if self.include_img:
                    # for mask_image_path in mask_image_list:
                    zipf.write(
                        mask_image_list[idx],
                        os.path.join("mask", "train", os.path.basename(mask_image_list[idx])),
                    )
                if self.include_instance:
                    # for mask_instance_path in mask_instance_list:
                    zipf.write(
                        mask_instance_list[idx],
                        os.path.join("mask", "train", os.path.basename(mask_instance_list[idx])),
                    )
                current_progress += 1
                progress_dialog.setValue(current_progress)
                if progress_dialog.wasCanceled():
            ...
            coco_json_path, image_path_list, mask_list, mask_image_list, mask_instance_list = (
                self._convert_to_coco_format(self.valid_file_list, "valid")
            )
            ...
            for idx, image_path in enumerate(image_path_list):
                ...
                if self.include_mask:
                    ...
                    zipf.write(
                        mask_list[idx],
                        os.path.join("mask", "valid", os.path.basename(mask_list[idx])),
                    )
                if self.include_img:
                    # for mask_image_path in mask_image_list:
                    zipf.write(
                        mask_image_list[idx],
                        os.path.join("mask", "valid", os.path.basename(mask_image_list[idx])),
                    )
                if self.include_instance:
                    # for mask_instance_path in mask_instance_list:
                    zipf.write(
                        mask_instance_list[idx],
                        os.path.join("mask", "valid", os.path.basename(mask_instance_list[idx])),
                    )
                current_progress += 1
                progress_dialog.setValue(current_progress)
                if progress_dialog.wasCanceled():
            ...
            coco_json_path, image_path_list, mask_list, mask_image_list, mask_instance_list = (
                self._convert_to_coco_format(self.test_file_list, "test")
            )
            ...
            for idx, image_path in enumerate(image_path_list):
                ...
                if self.include_mask:
                    ...
                    zipf.write(
                        mask_list[idx],
                        os.path.join("mask", "test", os.path.basename(mask_list[idx])),
                    )
                if self.include_img:
                    # for mask_image_path in mask_image_list:
                    zipf.write(
                        mask_image_list[idx],
                        os.path.join("mask", "test", os.path.basename(mask_image_list[idx])),
                    )
                if self.include_instance:
                    # for mask_instance_path in mask_instance_list:
                    zipf.write(
                        mask_instance_list[idx],
                        os.path.join("mask", "test", os.path.basename(mask_instance_list[idx])),
                    )
                current_progress += 1
                progress_dialog.setValue(current_progress)
                if progress_dialog.wasCanceled():
...
class ExportFileDialog(QDialog):
    ...
    def _update_train_test_validation(self):
        ...
        self.label.setText(
            f"Train: {self.training_percentage}% ({train})\n"
    ...
    def _on_checkbox_state_changed(self):
        ...
        if self.task_type == ANNOTATION_TYPE.POLYGON:
            if self.include_mask:
                ...
                self.include_instance_checkbox.stateChanged.connect(
                    self._on_instance_checkbox_state_changed
                )
    ...
    def _on_export(self):
        ...
        if (
            self.export_type == ExportType.YOLOV5
            or self.export_type == ExportType.YOLOV7
            or self.export_type == ExportType.YOLOV8
            or self.export_type == ExportType.YOLOV9
        ):
            yolo_conversion = YOLOConversion(
                self.save_dir,
                self.data_dir,
                self.file_list,
            ...
            if yolo_conversion.zip_filename:
                QMessageBox.information(
                    self, "Export Successful", f"Exported data to {yolo_conversion.zip_filename}"
                )
                self.accept()
                return
        elif self.export_type == ExportType.COCO:
            coco_conversion = COCOConversion(
                self.save_dir,
                self.data_dir,
                self.file_list,
                self.training_percentage,
            ...
            if coco_conversion.zip_filename:
                QMessageBox.information(
                    self, "Export Successful", f"Exported data to {coco_conversion.zip_filename}"
                )
                self.accept()
                return
        self.accept()
